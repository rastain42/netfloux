<template>
  <div class="gunun-populerleri">
    <div class="text-span">
      <span> Günün Popülerleri </span>
    </div>
    <swiper
      :slides-per-view="7"
      :space-between="0"
    ...
    >
      <swiper-slide v-for="dayPopular in dayPopular" :key="dayPopular.id">
        <div class="ms-2 me-2 movie_card item">
          <img
            :src="'http://image.tmdb.org/t/p/w500/' + dayPopular.poster_path"
            class="card-img-top"
            alt="..."
          />
          ...
          <div class="film_info" v-if="isAuthenticated">
            <div class="list-icon-left">
              ...
              <a @click="addFavorite(dayPopular)"
      ...
      try {
        await axios
          ...
          .then((response) => {
            ...
            this.modalData = dayPopular;
          });

        console.log("dayPopular", dayPopular);
        this.isOpen = true;
  ...
  created() {
    axios
      ...
      .then((response) => {
        ...
        this.dayPopular = response.data.results;
      });
  },
  computed: {
    ...mapGetters({
