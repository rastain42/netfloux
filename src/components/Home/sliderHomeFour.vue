        ...
        <div class="ms-2 me-2 movie_card item">
          ...
          <div class="film_info" v-if="isAuthenticated">
            <div class="list-icon-left">
              ...
              <a @click="addFavorite(newMovieList)"
...
export default {
  ...
  methods: {
    ...
    async showDetail(newMovieList) {
      ...
      try {
        await axios
          .get(
            `https://api.themoviedb.org/3/movie/${newMovieList.id}?api_key=b8da754884cd3ae20d385db99b95ff93&language=en-US&append_to_response=videos,credits,release_dates,similar`
          )
          .then((response) => {
  ...
  created() {
    axios
      .get(
        "https://api.themoviedb.org/3/movie/upcoming?api_key=b8da754884cd3ae20d385db99b95ff93&language=en-US&page=1"
      )
      .then((response) => {
