...
export default {
  ...
  methods: {
    ...
    async showDetail(OutOnerilenler) {
      ...
      try {
        await axios
          .get(
            `https://api.themoviedb.org/3/movie/${OutOnerilenler.id}?api_key=b8da754884cd3ae20d385db99b95ff93&language=en-US&append_to_response=videos,credits,release_dates,similar`
          )
          .then((response) => {
  ...
  created() {
    axios
      .get(
        "https://api.themoviedb.org/3/movie/38700/similar?api_key=<//APIKEY\\>&language=en-US&page=1"
      )
      .then((response) => {
